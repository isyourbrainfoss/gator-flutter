# Gator Flutter — Task list (2026-09-15)
...
plus upstream **croc v11.5.3** (Gator was still on **11.3.2** after the
v1.5.14 batch; 11.5 normalized the progressbar and send/receive UX).

Each task is **solved** only when its acceptance criteria pass.

| ID | Task | Solved when |
|----|------|-------------|
| T1 | Bump bundled croc to **11.5.3**; pin one version; send custom codes via `CROC_SECRET` not `--code`; parse v11.2.4+ `On the other computer, run: croc …` plus legacy `Code is:` | `crocVersion` / build scripts / metainfo are 11.5.3; `rg 11.0.1` only in this changelog; `buildSendArgs` has no `--code`; parser tests cover both code formats |
...
| T10 | Bump bundled croc **11.3.2 → 11.5.3**; parse v11.5 progressbar (filename, %, bytes, `kB`/`MB` speed, ETA) and `Sending (ip->ip)` / `Receiving (ip<-ip)` | Pins and Android asset are 11.5.3; parser tests cover v11.5 bars, P2P direction labels, clipboard-notice codes |
...
| ID | Status | Notes |
|----|--------|-------|
| T1 | Solved | Bundled croc 11.5.3; `CROC_SECRET` for send; dual code parsers |
...
| T9 | Solved | `flutter analyze` clean; `flutter test` green |
| T10 | Solved | croc 11.5.3; v11.5 progress/UX parser; CI Go 1.27 |
